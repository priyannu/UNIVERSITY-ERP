    ...
    <div className="page">
      ...
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        {[
          ...
          ["Resolved", posts.filter(p => p.status === "resolved").length, "blue", "✅"],
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 14 }}>
        ...
        {filtered.map(p => (
          <div key={p._id} className="card" style={{ borderLeft: `4px solid ${p.type === "lost" ? "#ef4444" : "#10b981"}`, opacity: p.status === "resolved" ? 0.6 : 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: "1.3rem" }}>{p.type === "lost" ? "😢" : "😊"}</span>
                <div>
                  <span style={{
                    fontSize: "0.7rem", fontWeight: 700, padding: "2px 8px", borderRadius: 999,
                    background: p.type === "lost" ? "#fee2e2" : "#d1fae5",
                    color: p.type === "lost" ? "#991b1b" : "#065f46",
                    textTransform: "uppercase"
                  }}>{p.type}</span>
                  {p.status === "resolved" && <span className="badge badge-success" style={{ marginLeft: 6 }}>Resolved</span>}
                </div>
              </div>
              <span style={{ fontSize: "0.72rem", color: "var(--text3)" }}>{new Date(p.createdAt).toLocaleDateString()}</span>
            </div>
            ...
            {p.status === "open" && (
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button className="btn btn-success btn-sm" onClick={() => resolve(p._id)}>Mark Resolved</button>
