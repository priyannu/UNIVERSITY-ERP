  ...
  const submitBulk = async () => {
    ...
    try {
      await Promise.all(bulkStudents.map(s =>
        API.post("/attendance", { studentId: s.id, studentName: s.name, subject: markForm.subject, date: markForm.date, status: s.status })
      ));
      alert(`Attendance marked for ${bulkStudents.length} students!`);
      setBulkStudents([]);
    } catch { alert("Some records failed"); }
  };
        ...
        <div className="card">
          ...
          {bulkStudents.length > 0 && (
            <>
              ...
              <button className="btn btn-primary" onClick={submitBulk}>Submit Attendance for {bulkStudents.length} Students</button>
            </>
          )}
